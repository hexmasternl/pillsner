...
**Persistence.** `critical` is a plain `Boolean` column on the `Medication` entity, default `false`, following the same migration pattern as any other schema change: a new Room migration plus its migration test. It is also added to `NewMedication`, the unsaved-form model `MedicationRepository.add` takes, alongside `Medication` — without it the Add form has no way to carry the toggle into the repository.

**Propagation to a dose: a third snapshot field, not a lookup.** `Dose` already snapshots `medicationName` and `amount` at generation time, deliberately, so a dose outlives edits to its medicine (design note on `Dose`). `critical` follows the same pattern: `Dose` and `PlannedDose` gain a `critical: Boolean` field, populated by `DoseGenerator.plan` from the medicine's current flag, the same way `medicationName` and `amount` already are. This is what lets every consumer that already holds a `Dose` — `DoseDetailViewModel` (dose detail badge), `ReminderNotifier.show(dose, ...)` (channel and cadence selection), and the wearable sync publisher (`DoseRepository.observePending()` payload mapping) — read the flag directly, with no new `MedicationRepository` lookup or join added to any of those three paths.

Keeping a pending dose's snapshot current when the user toggles the flag on an existing medicine reuses `RefreshPlannedDoses`' existing snapshot refresh (`doseRepository.refreshSnapshots(planned)`, which today keeps `medicationName`/`amount` following the medicine while a dose is still pending): `critical` is refreshed the same way, on the same "runs on every wake, on every medication change" cadence already documented there. Because the escalation cadence and channel are computed fresh from the dose's current `critical` value each time a repeat alarm is armed (not fixed once when the dose was first planned), a flag toggled after a dose was planned but before it was first reminded takes effect on the next computation. A dose already mid-repeat-sequence (already reminded at least once) keeps the cadence it was already announced under until it is answered or lapses — consistent with `RefreshPlannedDoses`' existing rule that an already-reminded dose is "a fact" that user edits do not silently rewrite outside the explicit `afterUserEdit` withdrawal path.

**Wear sync.** `wearable-sync`'s shared contract gains a `critical: Boolean` field per dose entry (mirroring the medicine it belongs to), defaulting to `false` when absent so an older payload without the field still deserialises under the existing "unknown field tolerated" / additive-only versioning rule. The watch shows the same badge; it makes no scheduling decision, so no cadence logic is needed on the watch. Toggling a medicine's flag republishes automatically, with no extra trigger to wire: `DoseSyncPublisher` already collects `DoseRepository.observePending()`, a Room `Flow` query that re-emits whenever the underlying `doses` table changes; once `RefreshPlannedDoses` writes the refreshed `critical` snapshot to that table (see above), the existing collector picks it up the same way it already does for a `medicationName`/`amount` change.
