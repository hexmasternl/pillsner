...
Constraints from `CLAUDE.md` that shape this design: domain layer without Android imports, strings in resources, Material 3 accessibility (large fonts, TalkBack, one hand), no new dependencies without a named reason, glossary terms (`Medication`, `Schedule`) in code, every visual decision from `docs/design-system.md` tokens through the `pillsner-ui-build` and `pillsner-ui-review` skills. UI copy uses "medicine" because that is the word already on the bottom navigation item; code uses `Medication` per the glossary. This change does not touch the project scaffold, which `app-welcome-screen` alone owns.
...
`MedicinesScreen` is a `Scaffold` whose `floatingActionButton` is a `LargeFloatingActionButton` per design system section 8.5: `primaryContainer` / `onPrimaryContainer`, the `add` icon, content description "Add medicine", bottom-end, `Spacing.lg` from the edges, above the navigation bar. Medicines is a top-level destination, so like Home it has no app bar; the title "Medicines" is rendered in content as `displayLarge` with `heading()` semantics (section 8.7). The content is a single `LazyColumn` with `Arrangement.spacedBy(Spacing.lg)`, `Spacing.screenEdge` side padding and the `Spacing.contentMaxWidth` cap, holding:

1. Section header "Active" in `headlineSmall` (section 3.2, group headers), shown whenever at least one medicine exists in either section.
2. Active tiles, or an active empty state per section 8.10 (outlined `medication` icon, headline "No active medicines", hint "Tap + to add one.") when `active` is empty but `inactive` is not.
3. Section header "Inactive" in `headlineSmall` and inactive tiles, only when `inactive` is non-empty.
4. When both lists are empty and loading is finished: the shared `EmptyState` composable from `app-welcome-screen`, centred, with the headline "No medicines yet" and the hint "Tap + to add your first medicine.", and no headers.

*Why one list rather than two:* two independently scrolling lists on one screen are hard to use with one hand and confusing with TalkBack. One list with section headers scrolls as a whole at large font sizes and gives a natural reading order. Headers use `stickyHeader` on the `surface` colour so the user always knows which section they are in.

The list has bottom `contentPadding` of the FAB height plus `Spacing.lg` so the last tile is never hidden behind the button (section 5). The FAB sits inside the Medicines screen's own `Scaffold`, so it stays above the bottom navigation bar.
...
`MedicineTile` follows design system section 8.2: the same `Card` as the dose tile (`shapes.large`, `tileContainerColor()`, no border, no shadow, padding `Spacing.lg`) without the state stripe. The name is `titleMedium` and each schedule description is a `bodyMedium` line. Inactive tiles use the `surfaceContainerLow` container with all text in `onSurfaceVariant`, a small `Inactive` chip on `surfaceContainerHighest` (`IntakeStatusChip` styling without a status), and `stateDescription = "Inactive"`. Alpha is never lowered, because it fails contrast; the section header, the chip and the spoken state carry the meaning, so the distinction does not rely on colour alone. Each tile uses `Modifier.semantics(mergeDescendants = true)` so TalkBack reads "Ibuprofen, twice a day" as one item. Minimum height `Sizes.tileMinHeight`; tiles are not clickable in this change but keep the standard card shape so tapping can be added without a layout change.
...
- [Sticky headers with a large FAB can crowd a small screen at maximum font scale] → Headers are `headlineSmall` and wrap rather than truncate, the FAB is anchored to the scaffold rather than the list, and the manual test case covers 200 percent font scale.
- [Inactive de-emphasis by colour only] → Section header, surface tier, the `Inactive` chip and the spoken state carry the distinction independently of colour, as design system section 8.2 requires.
