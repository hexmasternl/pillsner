...
Constraints from `CLAUDE.md`: domain layer without Android imports, strings in resources, Material 3 accessibility, single DI mechanism, never log medication names or dosages at info level or above, no destructive migration, no broad refactors bundled into a feature.
...
In `Edit` mode the form shows, above the schedules section, a `ListItem` with headline "Active", supporting text "Inactive medicines are not reminded", and a `Switch` bound to `draft.isActive`. It is absent in `Add` mode because new medicines are always active (add spec). The value is written as part of `update`, so a deactivation from the details screen and one from the swipe both end in the same repository stream emission, and the reminder refresh treats them identically. The switch has a spoken label and the standard toggleable semantics.
