...
### 3. Provision via a subscription-scoped Bicep template, then fetch a deployment token directly
`infra/website/main.bicep` (`targetScope = 'subscription'`) creates the resource group and delegates to `infra/website/static-web-app.bicep` (resource-group scope) to create the Free-tier `Microsoft.Web/staticSites` resource with no `repositoryUrl` (this workflow pushes content itself; the resource isn't wired to GitHub's own build-and-deploy integration). Both resources are declared directly (`= { ... }`, not `existing`), so `az deployment sub create` is idempotent: a first run creates everything, every later run is a no-op reconciliation.

The deployment token is fetched separately, via `az staticwebapp secrets list --name ... --resource-group ... --query "properties.apiKey" -o tsv`, run right after the Bicep deployment against the resource it just provisioned (or confirmed). This is the pattern Microsoft's own Static Web Apps CLI/CI-CD docs recommend for getting a deployment token without a portal round-trip.

**Alternative considered (tried and reverted):** have `static-web-app.bicep` expose `staticSite.listSecrets().properties.apiKey` as a `@secure()` output, propagated through the subscription-scoped parent template, and read it from `az deployment sub create`'s own `--query properties.outputs` response. This compiled cleanly and looked correct, but failed in practice: `swa deploy` rejected the value with `deployment_token provided was invalid`. The most likely cause is that ARM's secure-output handling doesn't round-trip the real secret value through a module boundary at this scope combination cleanly — the deployment's own CLI response gets something other than the live key. Rather than chase that mechanism further, the design now avoids routing the secret through Bicep outputs at all.

**Alternative considered:** authenticate with `azure/login`, then call `swa deploy` directly with `--subscription-id`/`--tenant-id`/`--client-id`, relying on the CLI's own AAD login. Rejected after an actual run: `swa deploy`'s AAD path is interactive-first — when it can't resolve the named app (because it doesn't exist yet), it drops into a resource picker prompt with no way to answer it on a non-interactive runner, so the job just hangs instead of failing.
