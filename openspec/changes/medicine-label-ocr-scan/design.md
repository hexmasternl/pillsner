...
- **Photo lifecycle: capture (or pick) → recognise → discard.** The `Uri` handed back by the camera intent or photo picker is read once for `InputImage` construction and never written to app storage. Any camera-intent temp file the capture needs is created only under `context.cacheDir` (never external or persistent storage), so the OS may reclaim it under storage pressure and it is never a candidate for backup. It is deleted immediately after recognition completes or fails, in a `finally` — but a `finally` does not run if the process is killed or crashes mid-recognition, so this is a best-effort prompt cleanup, not the guarantee against a leaked photo. That guarantee comes from cache-only placement plus a stale-file sweep: on every app start and at the beginning of every new scan, any file under the dedicated `cacheDir/label-scan/` subdirectory older than the current recognition attempt is deleted before proceeding. Between "a crash leaves a temp file behind" and "the next start or scan removes it," there is no window in which the file is exposed to another app — it was never outside the app's own cache.
- **Parsing heuristics live in the domain layer as a pure function.** `ParseMedicineLabelText(rawText: String): LabelScanResult` takes the raw recognised text block and returns a candidate name and an optional candidate dose (amount + unit token), in that order: first the dose is found by matching a `<number><space?><unit>` regex over the known unit list from `medicine-add` anywhere in the first non-empty line; then the name candidate is that same line with the matched dose token (and any trailing/leading punctuation or whitespace it leaves behind) removed, trimmed. A line reading "Metoprolol 40 mg" therefore yields the dose "40"/"mg" and the name "Metoprolol", not "Metoprolol 40 mg" — finding the dose first is what makes stripping it from the name candidate possible, rather than treating name and dose extraction as independent passes over the same line. When no dose token matches anywhere on the first non-empty line, the whole line is used as the name candidate unchanged, same as today. This keeps the parsing unit-testable with zero Android or ML Kit dependency, per CLAUDE.md's layering rule — only the camera call and the ML Kit `TextRecognizer` invocation live in the data/UI layer.
