...
`AddMedicationScreen` is a `Scaffold` with a `TopAppBar` (title "Add medicine", back arrow, `Save` text button in the actions) and a vertically scrolling `Column`:

1. **Name**: `OutlinedTextField`, single line, capitalised words, required.
2. **Default dose**: a `QuantityField` composable: numeric `OutlinedTextField` (decimal keyboard, locale-aware parsing) plus an `ExposedDropdownMenuBox` for the unit. Reused by the schedule editor.
...
6. **Schedules** section header, then one `ScheduleRow` per draft schedule (description via the formatter, tap to edit, trailing remove icon with content description "Remove schedule"), then a `Button` "Add schedule". When empty, a supporting line "No schedules yet. This medicine will be listed as taken when needed."

Save is enabled only when the draft is valid; invalid fields show `supportingText` errors on blur or on a Save attempt. On Save, the view model calls `repository.add`, then emits a one-shot `Saved` effect; `PillsnerApp` pops back to `Medicines`. The overview updates through its `Flow`.
...
`ScheduleEditorScreen` is a `Scaffold` with a `TopAppBar` (title "Add schedule" or "Edit schedule", back arrow, `Done` text button) and a scrolling `Column`:
...
3. Pattern inputs:
   - *Every N days*: an interval stepper (1 to 30, label reads "Every day" for 1, "Every other day" for 2, "Every N days" otherwise) and a **times list**: one row per `LocalTime` with a `TimePickerDialog` on tap and a remove icon, plus "Add time". At least one time; duplicates are rejected with a message.
   - *Weekdays*: seven `FilterChip`s in the locale's week order plus the same times list. At least one day; selecting all seven shows a hint to use "Every N days" and is rejected on Done so the model invariant holds.
   - *Every N hours*: an interval `ExposedDropdownMenuBox` limited to 1, 2, 3, 4, 6, 8, 12, 24, and a first-dose time picker. Below it, the computed dose times for a day ("08:00, 20:00") as read-only text.
4. **Preview**: a card showing the description the overview will display ("40 mg every 12 hours"), updated on every change; shows the first validation error instead when the draft is invalid.

Done validates, builds the `Schedule`, and either appends to or replaces at `index` in the draft, then pops back. Back discards editor edits without prompting because the editor is small and the form still holds the previous version.

### D7. Discarding a draft

Pressing back or the back arrow on the form with any field changed shows an `AlertDialog` "Discard this medicine?" with Discard and Keep editing. An untouched form pops immediately. The check compares the draft with its initial value.
...
Version catalog additions: `room` (runtime, ktx, compiler, testing), `ksp` plugin at the version matching the project's Kotlin. `room { schemaDirectory("$projectDir/schemas") }` in the app module. `androidx.compose.material3` date and time pickers come from the existing BOM. KSP is Google-maintained and is the only supported processor for Room with Kotlin; this is the justification `CLAUDE.md` asks for.
...
- [Date and time picker dialogs at large font on small screens] → Material 3 pickers handle scaling; the manual test case covers maximum font scale for both screens.
