...
- **WHEN** the form opens
- **THEN** name is empty, default dose amount is empty with a unit preselected, used since is today's date, use until is empty, prescribed by is "General practitioner", the schedules list is empty, and the critical toggle is off

#### Scenario: Marking a medicine critical
- **WHEN** the user turns on the critical toggle and saves
- **THEN** the medicine is saved with its critical flag set to true

## ADDED Requirements

### Requirement: Critical toggle is optional and off by default
The critical toggle SHALL default to off and SHALL NOT be required to save the form. Turning it on or off SHALL NOT affect any other field's validity.

#### Scenario: Save with the toggle untouched
- **WHEN** the user fills every required field, leaves the critical toggle untouched, and saves
- **THEN** the medicine is saved as not critical
