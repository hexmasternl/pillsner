...
Constraints from `CLAUDE.md` that shape this design: strings in resources, Material 3 accessibility (large fonts, TalkBack, one hand), no new dependencies without a named reason, domain layer without Android imports, never log medication names, single DI mechanism. The product promise "confirm in one tap, keep the data on the device" extends naturally to "stop a medicine in one gesture".
...
The revealed button has a text label and an icon, meets the 48 dp touch target, and uses `secondaryContainer` colours for Deactivate and `primaryContainer` for Activate. Neither uses the error palette: the action is reversible and not destructive. The label wraps at large font scales; the action area grows to fit its text with a minimum width of 96 dp so the label never clips.
...
- [`anchoredDraggable` API surface differs between Compose Foundation versions] → The version catalog pins the Compose BOM the project already uses; the wrapper is isolated in one file so an API rename is a one-file fix.
