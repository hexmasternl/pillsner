...
- **Depends on `app-welcome-screen`**: that change is the only owner of the project scaffold (Gradle project, version catalog, `MainActivity`, `PillsnerApp`, `PillsnerTheme`, `AppContainer`, the Settings destination). This change does not create any of it. Its first task verifies the shell exists and stops if it does not. The lock gate wraps the navigation host from that shell, `MainActivity` is changed to extend `FragmentActivity` for `BiometricPrompt`, and `AppContainer` is extended with the lock dependencies.
- **Settings screen**: the Security section is added to the Settings destination. If `app-settings-language` has already turned the placeholder into a list of sections, the Security section is inserted beneath Language; otherwise this change replaces the placeholder with the same sections structure so the two changes compose in either order.
- **Design system**: the unlock screen, keypad, PIN setup flow and Security section follow `docs/design-system.md`: theme tokens only, the primary action as a filled button of at least 56 dp in the bottom third, errors in the `error` role with icon and live region, switches in `ListItem` rows, and `pillsner-ui-review` run before the change is declared done.
- **Dependencies** (all AndroidX, first party, added to the existing version catalog): `androidx.biometric:biometric` 1.1.0 (newest stable; the 1.4.0 line is still alpha) for the biometric prompt and availability checks, `androidx.datastore:datastore-preferences` 1.2.1 for lock settings, `androidx.lifecycle:lifecycle-process` 2.11.0 for foreground/background detection. No third-party libraries, no network access, no telemetry.
