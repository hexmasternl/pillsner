...
### 3. Report a bug / Request a feature: client-side URL assembly, no backend
A small script, `assets/js/issue-report.js`, reads the form's fields on submit, builds `https://github.com/hexmasternl/pillsner/issues/new?title=<encoded>&body=<encoded>&labels=bug` (or `labels=feature`), and navigates the visitor there (`window.location.href`, same tab) to finish and submit with their own GitHub account. No fetch call, no token, nothing leaves the browser except the visitor's own navigation to GitHub.

**No-JS fallback:** the form's plain `action` attribute points at `https://github.com/hexmasternl/pillsner/issues/new?labels=bug` (or `feature`) directly, so a visitor without JavaScript still lands on a correctly-labelled new-issue page, just without the title/body pre-filled from their inputs.
