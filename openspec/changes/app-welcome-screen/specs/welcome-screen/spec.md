...
Each upcoming dose SHALL be rendered as a distinct tile that shows the medication name, the dose amount and the scheduled time. The scheduled time MUST be formatted for the device locale and time zone. When the dose is scheduled on a day other than today, the tile MUST also indicate the day.

#### Scenario: Dose scheduled later today
- **WHEN** a dose for "Ibuprofen", amount "1 tablet", is scheduled at 20:00 today
- **THEN** the tile shows "Ibuprofen", "1 tablet" and the time 20:00 in the device's time format, with no day indication
...
When there are no upcoming doses, the welcome screen SHALL show an empty-state message instead of an empty list. The message MUST come from a string resource.

#### Scenario: No upcoming doses
- **WHEN** the repository reports zero upcoming doses
- **THEN** no tiles are shown and an empty-state message is displayed below the header
