...
- **Dependencies** (first party): `androidx.core:core` 1.19.0 for `NotificationCompat` (already in the scaffold; `core-ktx` is merged into `core` since 1.19.0), `androidx.room` already added by `app-medicine-add`. No Wear OS, Play services or third-party libraries.
