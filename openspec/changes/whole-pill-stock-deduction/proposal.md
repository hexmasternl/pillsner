...
- Before first-expiry-first-out applies, the deduction prefers a batch that fits the dose exactly: its strength divides the dose evenly and it holds enough to cover it. If several batches fit, the one expiring soonest wins, and a tie goes to the batch added first. If none fits, first-expiry-first-out with whole-pill rounding applies as before.
