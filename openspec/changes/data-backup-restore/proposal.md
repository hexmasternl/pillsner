...
- **Export** writes a single versioned, encrypted `.pill` file via the Storage Access Framework document picker, containing every medication (active and inactive), every schedule, every recorded intake (taken, skipped, missed) and current stock/refill state. Device-only state (app lock PIN, accepted-legal-document timestamps, Wear OS pairing) is explicitly excluded, matching how `app-reset` already treats those as surviving separately from user data.
